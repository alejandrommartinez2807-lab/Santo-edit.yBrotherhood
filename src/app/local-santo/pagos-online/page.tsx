    ...
    <main className="min-h-screen bg-[var(--brand-cream)] px-4 py-8 text-[var(--brand-ink-2)]">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-serif text-2xl text-[var(--brand-ink-3)] font-semibold">Pagos / depósitos</h1>
        ...
        {denied ? (
          ...
          <>
            <div className="mt-6 grid gap-2 rounded-2xl border border-[var(--brand-primary)]/20 bg-white p-4 sm:grid-cols-2">
              ...
              <button onClick={record} disabled={busy || !reservationId || !(Number(amount) > 0)} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[var(--brand-primary)] px-4 py-3 text-sm font-bold uppercase text-white disabled:opacity-50 sm:col-span-2">
                  ...
                  <li key={p.id} className="rounded-2xl border border-[var(--brand-primary)]/20 bg-white p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {p.status === "reportado" && (
                          <>
                            <button onClick={() => post({ action: "status", id: p.id, status: "confirmado" })} disabled={busy} title="Confirmar" className="inline-flex items-center justify-center rounded-full border border-green-600/30 bg-green-50 p-1.5 text-green-700 disabled:opacity-50"><Check size={14} /></button>
                            <button onClick={() => post({ action: "status", id: p.id, status: "rechazado" })} disabled={busy} title="Rechazar" className="inline-flex items-center justify-center rounded-full border border-red-200 bg-white p-1.5 text-red-600 disabled:opacity-50"><X size={14} /></button>
                          </>
                        )}
                      </div>
                    </div>
