    ...
    <main className="min-h-screen bg-[var(--brand-cream)] px-4 py-8 text-[var(--brand-ink-2)]">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase text-[var(--brand-ink-3)]">Reseñas</h1>
        ...
        {denied ? (
          ...
          <>
            {summary && (
              <div className="mt-6 flex items-center gap-4 rounded-2xl border-2 border-[var(--brand-primary)]/20 bg-white p-4">
                <div className="text-center">
                  <p className="text-3xl font-black text-[var(--brand-ink-3)]">{summary.average || "—"}</p>
            ...
            {loading ? (
              ...
              <ul className="mt-6 space-y-3">
                {reviews.map((r) => (
                  <li key={r.id} className={`rounded-2xl border-2 bg-white p-4 ${r.published ? "border-[var(--brand-primary)]/20" : "border-[var(--brand-primary)]/10 opacity-60"}`}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="font-black text-[var(--brand-ink-3)]">{r.guestName} <Stars n={r.rating} /></p>
