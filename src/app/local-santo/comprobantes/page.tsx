        ...
        <div className="mx-auto max-w-7xl">
          <header className="overflow-hidden rounded-[2rem] border border-[var(--brand-primary)]/45 bg-white shadow-sm">
            ...
            <div className="grid gap-5 px-5 py-6 lg:grid-cols-[1fr_auto] lg:items-center lg:px-7">
              ...
              <button
                type="button"
                onClick={() => loadProofs()}
                className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-accent)] px-5 py-4 text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-ink)] shadow-sm transition hover:bg-[var(--brand-accent-200)] active:translate-y-1 active:shadow-none"
              >
          ...
          <section className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-[1.5rem] border border-[var(--brand-primary)]/40 bg-white p-4 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--brand-primary)]">Por revisar</p>
              <p className="mt-2 text-4xl font-bold text-[var(--brand-ink)]">{getPendingCount(proofs)}</p>
            </div>
            <div className="rounded-[1.5rem] border border-emerald-600 bg-emerald-50 p-4 shadow-[0_6px_0_rgba(16,185,129,0.12)]">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Confirmados</p>
              <p className="mt-2 text-4xl font-bold text-emerald-800">{getConfirmedCount(proofs)}</p>
            </div>
            <div className="rounded-[1.5rem] border border-orange-500 bg-orange-50 p-4 shadow-[0_6px_0_rgba(249,115,22,0.12)]">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700">Observados</p>
              <p className="mt-2 text-4xl font-bold text-orange-800">{getRejectedCount(proofs)}</p>
            </div>
          </section>

          <section className="sticky top-0 z-20 mt-5 rounded-[1.5rem] border border-[var(--brand-primary)]/40 bg-white p-4 shadow-sm">
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
              <label className="relative block">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] py-4 pl-12 pr-4 text-sm font-bold text-[var(--brand-ink)] outline-none placeholder:text-[var(--brand-ink)]/45 focus:border-[var(--brand-primary)]"
                />
              </label>
              ...
              <select
                ...
                className="rounded-2xl border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] px-4 py-4 text-sm font-bold text-[var(--brand-ink)] outline-none focus:border-[var(--brand-primary)]"
              >
          ...
          {message ? (
            <div className="mt-5 rounded-2xl border border-emerald-600 bg-emerald-100 px-4 py-3 text-sm font-bold text-emerald-800">
              {message}
            </div>
          ) : null}

          {error ? (
            <div className="mt-5 rounded-2xl border border-red-600 bg-red-100 px-4 py-3 text-sm font-bold text-red-800">
              {error}
            </div>
          ) : null}

          {isLoading ? (
            <section className="mt-5 flex min-h-72 items-center justify-center rounded-[2rem] border border-[var(--brand-primary)]/40 bg-white p-8 text-[var(--brand-primary)] shadow-sm">
              <Loader2 className="animate-spin" size={34} />
            </section>
          ) : filteredProofs.length === 0 ? (
            <section className="mt-5 rounded-[2rem] border border-[var(--brand-primary)]/40 bg-white p-8 text-center shadow-sm">
              <UploadCloud className="mx-auto text-[var(--brand-primary)]" size={44} />
              <h2 className="font-serif mt-4 text-2xl text-[var(--brand-ink-3)] font-semibold">Sin comprobantes</h2>
            ...
            <section className="mt-5 grid gap-4">
              {filteredProofs.map((proof) => {
                ...
                return (
                  <article
                    key={proof.id}
                    className="overflow-hidden rounded-[1.8rem] border border-[var(--brand-primary)]/40 bg-white shadow-sm"
                  >
                    <div className="grid gap-0 lg:grid-cols-[280px_1fr]">
                      <div className="border-b border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] p-4 lg:border-b-0 lg:border-r">
                        {proof.proofImageUrl ? (
                          <a href={proof.proofImageUrl} target="_blank" rel="noreferrer" className="group block">
                            <Image
                              ...
                              height={288}
                              unoptimized
                              className="h-72 w-full rounded-[1.25rem] border border-[var(--brand-primary)]/25 object-cover transition group-hover:scale-[1.01]"
                            />
                            <span className="mt-3 inline-flex items-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-white px-4 py-2 text-xs font-bold uppercase text-[var(--brand-primary)]">
                              <Eye size={15} />
                              Ver captura
                            </span>
                          </a>
                        ) : (
                          <div className="flex h-72 items-center justify-center rounded-[1.25rem] border border-dashed border-[var(--brand-primary)]/30 bg-white text-center text-sm font-bold text-[var(--brand-primary)]/60">
                            Sin imagen
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-4 sm:p-5">
                        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              <span className={`rounded-full border px-3 py-1 text-[0.68rem] font-bold uppercase ${statusClasses(proof.status)}`}>
                                {proof.status}
                              </span>
                              <span className="rounded-full border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] px-3 py-1 text-[0.68rem] font-bold uppercase text-[var(--brand-ink)]">
                                {proof.id}
                              </span>
                              {hasSiblingProofs && (
                                <span className="rounded-full border border-orange-500 bg-orange-100 px-3 py-1 text-[0.68rem] font-bold uppercase text-orange-800">
                                  ⚠ Este pedido tiene{" "}
                            ...
                            <h2 className="font-serif mt-3 text-2xl text-[var(--brand-ink-3)] font-semibold">
                          ...
                          <div className="rounded-2xl border border-[var(--brand-primary)]/20 bg-[var(--brand-cream)] px-4 py-3 text-sm font-bold text-[var(--brand-ink)]">
                        ...
                        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                          <div className="rounded-2xl border border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Fecha</p>
                            <p className="mt-1 text-sm font-bold text-[var(--brand-ink)]">{formatDateTime(proof.createdAt)}</p>
                          </div>
                          <div className="rounded-2xl border border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Método</p>
                            <p className="mt-1 text-sm font-bold text-[var(--brand-ink)]">{proof.reportedMethod || "Sin método"}</p>
                          </div>
                          <div className="rounded-2xl border border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Referencia</p>
                            <p className="mt-1 text-sm font-bold text-[var(--brand-ink)]">{proof.paymentReference || "Sin referencia"}</p>
                          </div>
                          <div className="rounded-2xl border border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Pedido</p>
                        ...
                        {proof.customerNote ? (
                          <div className="mt-4 rounded-2xl border border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Nota del cliente</p>
                        ...
                        {proof.reviewedAt ? (
                          <div className="mt-4 rounded-2xl border border-[var(--brand-primary)]/15 bg-white px-4 py-3">
                            <p className="text-[0.65rem] font-bold uppercase text-[var(--brand-primary)]">Última revisión</p>
                        ...
                        <label className="mt-4 block">
                          <span className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--brand-primary)]">Nota interna para esta revisión</span>
                          <textarea
                            ...
                            className="mt-2 min-h-20 w-full resize-none rounded-2xl border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] px-4 py-3 text-sm font-bold text-[var(--brand-ink)] outline-none placeholder:text-[var(--brand-ink)]/45 focus:border-[var(--brand-primary)]"
                          />
                        </label>
                        ...
                        <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-full border border-blue-700 bg-blue-100 px-4 py-3 text-xs font-bold uppercase text-blue-800 transition hover:bg-blue-200 disabled:opacity-60"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-full border border-emerald-700 bg-emerald-100 px-4 py-3 text-xs font-bold uppercase text-emerald-800 transition hover:bg-emerald-200 disabled:opacity-60"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-full border border-orange-600 bg-orange-100 px-4 py-3 text-xs font-bold uppercase text-orange-800 transition hover:bg-orange-200 disabled:opacity-60"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-full border border-red-700 bg-red-100 px-4 py-3 text-xs font-bold uppercase text-red-800 transition hover:bg-red-200 disabled:opacity-60"
                          >
