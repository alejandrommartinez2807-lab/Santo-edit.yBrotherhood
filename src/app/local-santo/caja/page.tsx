...
import type {
  LocalOrder as StoredLocalOrder,
  OpenAccount,
} from "@/types/localOrders";
import ModuleAccessGuard from "@/components/ModuleAccessGuard";
import { normalizeBusinessComplexitySettings } from "@/lib/orders";
...
import {
  ...
  isDeliveryOrder,
  isDeliveryReported,
  isOpenAccountEligibleLocalOrder,
...
function CajaPageContent() {
  ...
  const [localTables, setLocalTables] =
    useState<LocalTableMapItem[]>(DEFAULT_LOCAL_TABLES);
  const [canCancelOrders, setCanCancelOrders] = useState(true);
  ...
  const [paymentProofsMessage, setPaymentProofsMessage] = useState<
    string | null
  >(null);
  const [confirmingStaffOrderId, setConfirmingStaffOrderId] = useState<
    string | null
  >(null);
  const [attachingOpenAccountOrderId, setAttachingOpenAccountOrderId] =
  ...
  async function loadBusinessPermissions(password = adminPassword) {
    if (!password) {
      setCanCancelOrders(true);
      return;
    }
    ...
    try {
      ...
      setCanCancelOrders(permissions.internalAllowCancelOrders);
    } catch {
      setCanCancelOrders(true);
    }
  }
  ...
  async function loadOrders(password = adminPassword, silent = false) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          data.error || "No se pudieron cargar los pedidos de caja",
        );
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "No se pudieron cargar los pedidos de caja",
      );
    } finally {
  ...
  async function attachOrderToOpenAccount(
    order: LocalOrder,
    account: OpenAccount,
  ) {
    ...
    if (!cleanPassword) {
      setErrorMessage(
        "Ingresa la clave de caja para asociar el pedido a la cuenta.",
      );
      return;
    }
    ...
    try {
      ...
      setAccountMessage(
        `Pedido asociado a la cuenta abierta de ${account.tableNumber}.`,
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error
  ...
  async function updateStatus(orderId: string, status: OrderStatus) {
    ...
    if (status === "Cancelado" && !canCancelOrders) {
      setErrorMessage(
  ...
  function handleLogin() {
    ...
    loadLocalTables();
    loadBusinessPermissions(password);
    loadOrders(password);
  ...
  useEffect(() => {
    ...
    if (savedPassword) {
      ...
      loadBusinessPermissions(savedPassword);
    ...
    return orders
      .filter((order) => {
        ...
        if (activeFilter === "Por revisar") {
          return (
            order.status !== "Cancelado" && hasStaffConfirmationItems(order)
          );
        }
        ...
        if (activeFilter === "Pago parcial") {
          return (
            order.status !== "Cancelado" && payment.status === "Pago parcial"
          );
        }
      ...
      .filter((order) => {
        ...
        const searchableText = [
          ...
          getStaffConfirmationStatusLabel(
            getOrderStaffConfirmationSummary(order).status,
          ),
  ...
  const pendingPaymentCount = orders.filter(
    (order) =>
      getOrderPayment(order).status === "Pendiente" &&
      order.status !== "Cancelado",
  ).length;
  const partialPaymentCount = orders.filter(
    (order) =>
      getOrderPayment(order).status === "Pago parcial" &&
      order.status !== "Cancelado",
  ).length;
  const readyCount = orders.filter((order) => order.status === "Listo").length;
  const deliveryCount = orders.filter(
    (order) => isDeliveryOrder(order) && order.status !== "Cancelado",
  ).length;
  ...
  const filteredPendingUSD = filteredOrders.reduce(
  ...
  const paymentDraft = paymentModalOrder
    ...
    : null;
  const paymentOpenAccount = findOpenAccountForOrder(
    paymentModalOrder,
    openAccounts,
  );
  const paymentOpenAccountOrderCount =
    getOpenAccountOrderCount(paymentOpenAccount);
  const currentPaymentForModal = paymentModalOrder
    ? getOrderPayment(paymentModalOrder)
    : null;
  const paymentOpenAccountPendingBeforeDraft =
    getOpenAccountPendingUSD(paymentOpenAccount);
  const paymentOpenAccountTotalBeforeDraft =
    getOpenAccountTotalUSD(paymentOpenAccount);
  const paymentOpenAccountPendingAfterDraft =
    paymentOpenAccount && paymentDraft && currentPaymentForModal
      ? roundMoney(
          Math.max(
            paymentOpenAccountPendingBeforeDraft -
              currentPaymentForModal.pendingUSD +
              paymentDraft.pendingUSD,
            0,
          ),
        )
      : 0;
  const paymentOpenAccountCollectedAfterDraft =
    paymentOpenAccount && paymentDraft && currentPaymentForModal
      ? roundMoney(
          Math.max(
            Number(paymentOpenAccount.totalCollectedUSD || 0) -
              currentPaymentForModal.receivedEquivalentUSD +
              paymentDraft.receivedEquivalentUSD,
            0,
          ),
        )
      : 0;
          ...
          <div className="p-4 sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border-2 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] transition ${pendingPaymentProofsCount > 0 ? "border-[var(--brand-primary)] bg-[var(--brand-accent)] text-[var(--brand-ink)] hover:bg-[var(--brand-accent-200)]" : "border-[var(--brand-primary)] bg-white text-[var(--brand-primary)] hover:bg-[var(--brand-accent-100)]"}`}
                  >
                    <UploadCloud size={16} />
                    Comprobantes
                ...
                <p className="mt-3 max-w-2xl text-sm font-bold leading-6 text-[var(--brand-ink-2)]/70">
                  Caja confirma pedidos, registra pagos y decide cuándo enviar a
                  cocina. Cuando cocina marca listo, caja puede avisar la salida
                  y cerrar la entrega.
                </p>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-6 lg:w-[900px]">
                ...
                <MetricCard
                  label="Por revisar"
                  value={pendingStaffReviewCount}
                  tone={pendingStaffReviewCount > 0 ? "yellow" : "soft"}
                />
                <MetricCard
                  label="Parciales"
                ...
                <MetricCard
                  label="Comprobantes"
                  value={pendingPaymentProofsCount}
                  tone={pendingPaymentProofsCount > 0 ? "yellow" : "soft"}
                />
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                {pendingPaymentProofsCount > 0 ? (
                  <p className="mt-2 text-sm font-bold leading-6 text-[var(--brand-ink-2)]/75">
                    ...
                    {latestPendingPaymentProof
                      ...
                      : ""}
                    Confirmar un comprobante no registra el cobro real; el pago
                    se sigue guardando desde caja.
                  </p>
                ) : (
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  disabled={isLoadingPaymentProofs}
                  className="inline-flex items-center gap-2 rounded-full border-2 border-[var(--brand-primary)] bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-[var(--brand-primary)] transition hover:bg-yellow-50 disabled:opacity-50"
                >
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-bold leading-5 text-[var(--brand-ink-2)]/65">
                Filtra caja por ubicación. Puedes ocultarlo cuando la caja
                necesite más espacio para cobrar.
              </p>
            </div>
            <button
              type="button"
        ...
        <section className="mt-4 rounded-[1.4rem] border-2 border-[var(--brand-primary)]/25 bg-white p-3 shadow-[0_8px_0_rgba(var(--brand-primary-rgb),0.08)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-bold leading-5 text-[var(--brand-ink-2)]/65">
                Se mantiene oculto por defecto para que no estorbe durante la
          ...
          {showQrLinks ? (
            ...
            <p className="mt-3 rounded-2xl border-2 border-dashed border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] px-4 py-3 text-sm font-bold text-[var(--brand-ink-2)]/70">
              QR ocultos. Usa “Mostrar QR” solo cuando necesites copiar,
              imprimir o revisar enlaces de mesa.
            </p>
          )}
        </section>
        ...
        <section className="sticky top-0 z-30 mt-4 rounded-[1.4rem] border-2 border-[var(--brand-primary)] bg-white p-3 shadow-[0_8px_0_rgba(var(--brand-primary-rgb),0.10)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-bold text-[var(--brand-ink-2)]/65">
                {filteredOrders.length} pedido(s) en pantalla · Pendiente
                visible {formatUSD(filteredPendingUSD)} · {activeFilter}
                {selectedCashTableName
                  ? ` · Mesa: ${selectedCashTableName}`
                  : ""}
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={isLoading}
                className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-[var(--brand-primary)] bg-[var(--brand-accent)] px-5 py-3 text-xs font-black uppercase tracking-[0.12em] text-[var(--brand-ink)] transition hover:bg-[var(--brand-accent-200)] disabled:opacity-50"
              >
                {isLoading ? (
          ...
          {showControls ? (
            <div className="mt-3 space-y-3">
              <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Buscar por pedido, cliente, teléfono, mesa, zona o producto"
                  ...
                  {CASH_FILTERS.map((filter) => {
                    ...
                    return (
                      <button
                        ...
                        className={`shrink-0 rounded-full border-2 px-4 py-3 text-xs font-black uppercase tracking-[0.1em] transition ${isActive ? "border-[var(--brand-primary)] bg-[var(--brand-accent)] text-[var(--brand-ink)]" : "border-[var(--brand-primary)] bg-white text-[var(--brand-primary)] hover:bg-[var(--brand-accent-100)]"}`}
                      >
                        {filter}
                      </button>
                    );
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                <MiniMetric
                  ...
                  value={formatUSD(filteredPendingUSD)}
                />
            ...
            <div className="mt-3 rounded-2xl border-2 border-[var(--brand-primary)]/15 bg-[var(--brand-cream)] px-4 py-3 text-sm font-bold text-[var(--brand-ink-2)]/70">
              Filtros ocultos · {filteredOrders.length} pedido(s) en pantalla ·
              Pendiente visible {formatUSD(filteredPendingUSD)} · {activeFilter}
          ...
          <section className="mt-5 grid gap-4 xl:grid-cols-2">
            {filteredOrders.map((order) => (
              <CashOrderCard
                ...
                suggestedOpenAccount={findSuggestedOpenAccountForOrder(
                  order,
                  openAccounts,
                )}
                onAttachToSuggestedOpenAccount={(account) =>
                  attachOrderToOpenAccount(order, account)
                }
                ...
                isAttachingToOpenAccount={
                  attachingOpenAccountOrderId === order.id
                }
              />
            ))}
          </section>
          ...
          <div className="space-y-4">
            <div className="rounded-[1.4rem] border-2 border-[var(--brand-primary)]/25 bg-white p-4">
              ...
              <p className="mt-2 text-sm font-bold leading-6 text-[var(--brand-ink-2)]/70">
                Registra el dinero recibido por caja. El sistema calcula si
                queda pagado, parcial o pendiente.
              </p>
            </div>
              ...
              <div className="rounded-[1.4rem] border-2 border-green-600 bg-green-50 p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm font-bold leading-6 text-[#234000]">
                      Este cobro pertenece a la cuenta de{" "}
                      {paymentOpenAccount.tableNumber ||
                        paymentModalOrder.tableNumber}
                      . Caja está cobrando este pedido seleccionado, no cerrando
                      la cuenta completa. El total de la cuenta se recalcula con
                      los cobros reales guardados por pedido.
                    </p>
                  </div>
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-4">
                  <InfoBox
                    ...
                    value={String(paymentOpenAccountOrderCount)}
                  />
                  <InfoBox
                    label="Total cuenta"
                    value={formatUSD(paymentOpenAccountTotalBeforeDraft)}
                  />
                  <InfoBox
                    label="Cobrado al guardar"
                    value={formatUSD(paymentOpenAccountCollectedAfterDraft)}
                  />
                  <InfoBox
                    label="Pendiente al guardar"
                    value={formatUSD(paymentOpenAccountPendingAfterDraft)}
                  />
                </div>
                ...
                <div className="mt-3 rounded-2xl border-2 border-yellow-500 bg-yellow-50 px-3 py-3 text-xs font-bold leading-5 text-[var(--brand-ink)]">
                  Cerrar la cuenta no registra cobro ni marca pedidos como
                  pagados. Revisa el pendiente antes de cerrar
                  administrativamente desde el panel de cuentas.
                </div>

                {Array.isArray(paymentOpenAccount.orders) &&
                  paymentOpenAccount.orders.length > 0 && (
                    <div className="mt-3 rounded-2xl border border-green-600/25 bg-white px-3 py-3">
                      <p className="text-[0.65rem] font-black uppercase tracking-[0.16em] text-green-700">
                        Pedidos asociados
                      </p>
                      <div className="mt-2 space-y-1">
                        {paymentOpenAccount.orders
                          .slice(0, 4)
                          .map((accountOrder) => (
                            <div
                              key={accountOrder.id}
                              className="flex items-center justify-between gap-3 text-xs font-black text-[#234000]"
                            >
                              <span>
                                {accountOrder.displayNumber || accountOrder.id}{" "}
                                · {accountOrder.status}
                              </span>
                              <span>
                                {formatUSD(
                                  Number(accountOrder.pendingUSD || 0),
                                )}{" "}
                                pendiente
                              </span>
                            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <InputBox
                label="Monto recibido en divisas"
                value={paymentForm.amountReceivedUSD}
                onChange={(value) =>
                  updatePaymentForm("amountReceivedUSD", value)
                }
              ...
              <SelectBox
                ...
                onChange={(value) =>
                  updatePaymentForm("paymentMethodUSD", value)
                }
                options={PAYMENT_METHOD_USD_OPTIONS}
                emptyLabel="Sin registrar"
              />
              <InputBox
                label="Monto recibido en bolívares reales"
                value={paymentForm.amountReceivedVES}
                onChange={(value) =>
                  updatePaymentForm("amountReceivedVES", value)
                }
                placeholder="Ej: 1569.25 o 1569,25"
                helper="Escribe el monto real en bolívares, no el equivalente en dólares."
              />
              <SelectBox
                label="Método en bolívares"
                value={paymentForm.paymentMethodVES}
                onChange={(value) =>
                  updatePaymentForm("paymentMethodVES", value)
                }
