        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-4xl font-bold leading-none text-[var(--brand-primary)]">{getDisplayOrderNumber(order)}</p>
              <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold uppercase ${getStatusStyle(order.status)}`}>{getStatusIcon(order.status)}{order.status === "Nuevo" ? "Por confirmar" : order.status}</span>
              <span className={`inline-flex rounded-full px-3 py-1.5 text-xs font-bold uppercase ${getPaymentStatusStyle(payment.status)}`}>{payment.status}</span>
              {isDelivery && <span className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-primary)] px-3 py-1.5 text-xs font-bold uppercase text-white"><Truck size={15} />Delivery</span>}
              {deliveryReported && <span className="inline-flex items-center gap-2 rounded-full bg-green-100 px-3 py-1.5 text-xs font-bold uppercase text-green-700"><PackageCheck size={15} />Entrega reportada</span>}
          ...
          <div className="rounded-2xl border border-[var(--brand-primary)]/40 bg-white px-4 py-3 text-right">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--brand-primary)]">Total</p>
            <p className="mt-1 text-2xl font-bold text-[var(--brand-ink-3)]">{formatUSD(orderTotals.totalUSD)}</p>
            {payment.pendingUSD > 0 && <p className="mt-1 text-xs font-bold text-red-700">Pendiente {formatUSD(payment.pendingUSD)}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 p-4">
        {order.attachmentImageUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-3 rounded-[1.4rem] border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] p-3 transition hover:bg-[var(--brand-accent-100)]"
          >
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                {order.staffConfirmationUpdatedAt && (
                  <p className="mt-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--brand-ink-2)]/55">
              ...
              {hasPendingStaffConfirmation ? (
                <button
                  ...
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-primary)] px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-white transition hover:bg-[var(--brand-primary-dark)] disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={isConfirmingStaff}
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-green-600 bg-white px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-green-700 transition hover:bg-green-100 disabled:opacity-50"
                >
        ...
        {deliveryReported && (
          <div className="rounded-[1.4rem] border border-green-600 bg-green-50 p-4">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-green-700">Delivery por confirmar</p>
        ...
        {suggestedOpenAccount && (
          <div className="rounded-[1.4rem] border border-yellow-500 bg-yellow-50 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
              ...
              <button
                ...
                className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-accent)] px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-ink)] transition hover:bg-[var(--brand-accent-200)] disabled:opacity-50"
              >
        ...
        {isDelivery && (
          <div className="rounded-[1.4rem] border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] p-4">
            <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]"><Truck size={16} />Datos delivery</p>
            ...
            {phone ? (
              ...
              <p className="mt-3 rounded-2xl border border-yellow-400 bg-[var(--brand-accent-100)] px-3 py-2 text-xs font-bold text-[var(--brand-amber)]">Este delivery no tiene teléfono válido para WhatsApp.</p>
            )}
          </div>
        )}

        <div className="rounded-[1.4rem] border border-[var(--brand-primary)]/25 bg-white p-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]">Productos</p>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={onOpenPayment} className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-accent)] px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-ink)] transition hover:bg-[var(--brand-accent-200)]">
          ...
          {order.status === "Nuevo" && !hasPendingStaffConfirmation && (
            <button type="button" onClick={onSendToKitchen} className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-primary)] px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-white transition hover:bg-[var(--brand-primary-dark)]">
              <Send size={17} /> Pedido confirmado / enviar a cocina
            </button>
          )}

          {order.status === "Preparando" && (
            <div className="rounded-full border border-orange-400 bg-orange-100 px-5 py-3 text-center text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-amber)]">
              En cocina
            </div>
          )}
          ...
          {order.status === "Listo" && (
            hasPendingStaffConfirmation ? (
              ...
              <button type="button" onClick={onMarkDelivered} className="inline-flex items-center justify-center gap-2 rounded-full border border-green-600 bg-green-500 px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-white transition hover:bg-green-400">
          ...
          {order.status === "Entregado" && (
            <div className="rounded-full border border-green-600 bg-green-50 px-5 py-3 text-center text-xs font-bold uppercase tracking-[0.12em] text-green-700">
              Pedido entregado
            </div>
          )}

          {order.status === "Cancelado" && (
            <div className="rounded-full border border-[var(--brand-ink-3)] bg-[var(--brand-ink-3)] px-5 py-3 text-center text-xs font-bold uppercase tracking-[0.12em] text-white">
              Pedido cancelado
            </div>
          )}
          ...
          {order.status !== "Cancelado" && order.status !== "Entregado" && (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--brand-ink-3)] bg-white px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-ink-3)] transition hover:bg-red-50 hover:text-red-700"
            >
              <XCircle size={17} />
              Cancelar pedido
            </button>
...
export function WhatsAppButton({ href, label, dark, green }: { href: string; label: string; dark?: boolean; green?: boolean }) {
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className={`inline-flex items-center justify-center gap-2 rounded-full border px-4 py-3 text-center text-[0.68rem] font-bold uppercase tracking-[0.1em] transition ${className}`}>
...
export function ProductGroup({ title, items, exchangeRate, onlyCurrency }: { title: string; items: CartItem[]; exchangeRate: number; onlyCurrency?: boolean }) {
  return (
    <div className="rounded-2xl border border-[var(--brand-primary)]/20 bg-[var(--brand-cream)] p-3">
      <p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-[var(--brand-primary)]">{title}</p>
          ...
          return (
            <div key={`${item.id}-${item.name}-${index}`} className="rounded-xl bg-white px-3 py-2 text-sm font-bold text-[var(--brand-ink-3)]">
              <div className="flex items-start justify-between gap-3">
                ...
                <p className="shrink-0 font-bold text-[var(--brand-primary)]">{formatUSD(subtotal)}</p>
              </div>
              <p className="mt-1 text-xs font-bold text-[var(--brand-ink-2)]/60">{onlyCurrency ? "Base en divisas" : `Bs ${formatVES(subtotalVES)}`}</p>
              {pendingStaffReview && (
                <p className="mt-1 inline-flex rounded-full bg-[var(--brand-accent-100)] px-3 py-1 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[var(--brand-amber)]">
...
export function InputBox({ label, value, onChange, placeholder, helper }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; helper?: string }) {
  return (
    <div>
      <label className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]">{label}</label>
      <input type="text" inputMode="decimal" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-2 w-full rounded-2xl border border-[var(--brand-primary)]/25 bg-white px-4 py-4 text-base font-bold text-[var(--brand-ink)] outline-none focus:border-[var(--brand-primary)]" />
...
export function SelectBox({ label, value, onChange, options, emptyLabel = "Sin registrar" }: { label: string; value: string; onChange: (value: string) => void; options: string[]; emptyLabel?: string }) {
  return (
    <div>
      <label className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]">{label}</label>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-2xl border border-[var(--brand-primary)]/25 bg-white px-4 py-4 text-base font-bold text-[var(--brand-ink)] outline-none focus:border-[var(--brand-primary)]">
...
export function MetricCard({ label, value, tone = "red" }: { label: string; value: string | number; tone?: "red" | "yellow" | "soft" }) {
  ...
  return <div className={`min-w-0 overflow-hidden rounded-[1.2rem] border p-3 ${style}`}><p className="break-words text-[0.6rem] font-bold uppercase leading-4 tracking-[0.08em]">{label}</p><p className="mt-1 text-2xl font-bold">{value}</p></div>
}

export function MiniMetric({ label, value }: { label: string; value: string | number }) {
  return <div className="rounded-2xl border border-[var(--brand-primary)]/20 bg-[var(--brand-cream)] px-3 py-2"><p className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-[var(--brand-primary)]">{label}</p><p className="mt-1 break-words text-sm font-bold text-[var(--brand-ink-3)]">{value}</p></div>
}

export function InfoBox({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[1.2rem] border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] p-3"><p className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-[var(--brand-primary)]">{label}</p><p className="mt-1 break-words text-sm font-bold text-[var(--brand-ink-3)]">{value || "—"}</p></div>
}
...
export function ModalShell({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[var(--brand-ink-3)]/60 px-4 py-4 backdrop-blur-sm sm:items-center">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-[var(--brand-primary)]/45 bg-[var(--brand-cream)] text-[var(--brand-ink-3)] shadow-2xl shadow-black/45">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-[var(--brand-primary)] bg-white px-6 py-5">
          <h2 className="font-serif text-3xl leading-tight text-[var(--brand-ink-3)] font-semibold">{title}</h2>
          <button type="button" onClick={onClose} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-accent)] text-[var(--brand-ink)]"><X size={24} /></button>
        </div>
    ...
    <main className="flex min-h-screen items-center justify-center bg-[var(--brand-cream)] px-4 py-8 text-[var(--brand-ink-3)]">
      <div className="w-full max-w-md overflow-hidden rounded-[2rem] border border-[var(--brand-primary)]/45 bg-white shadow-sm">
        ...
        <div className="px-6 py-6">
          <a href="/admin" className="inline-flex items-center gap-2 rounded-full border border-[var(--brand-primary)]/40 bg-white px-4 py-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--brand-primary)]"><ArrowLeft size={16} />Volver</a>
          ...
          <p className="mt-5 text-center text-xs font-bold uppercase tracking-[0.28em] text-[var(--brand-primary)]">Acceso privado</p>
          <h1 className="font-serif mt-2 text-center text-4xl leading-tight text-[var(--brand-ink-3)] font-semibold">{title}</h1>
          <p className="mt-3 text-center text-sm font-bold leading-6 text-[var(--brand-ink-2)]/75">{subtitle}</p>
        </div>
        <div className="space-y-4 px-6 pb-6">
          <div>
            <label className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]">Clave de acceso</label>
            <div className="relative mt-2">
              <input type={showPassword ? "text" : "password"} value={passwordInput} onChange={(event) => setPasswordInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") handleLogin() }} placeholder="Ingresa la clave del local" className="w-full rounded-2xl border border-[var(--brand-primary)]/25 bg-[var(--brand-cream)] px-4 py-4 pr-12 text-base font-bold text-[var(--brand-ink)] outline-none placeholder:text-[var(--brand-ink)]/45 focus:border-[var(--brand-primary)]" />
          ...
          {errorMessage && <div className="rounded-2xl border border-red-500/35 bg-red-100 px-4 py-3"><p className="text-sm font-bold leading-6 text-red-800">{errorMessage}</p></div>}
          <button type="button" onClick={handleLogin} className="flex w-full items-center justify-center gap-3 rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-accent)] px-6 py-4 text-sm font-bold uppercase tracking-[0.12em] text-[var(--brand-ink)] shadow-sm transition hover:scale-[1.02]"><LogIn size={21} />Entrar</button>
        </div>
      </div>
    </main>
  )
