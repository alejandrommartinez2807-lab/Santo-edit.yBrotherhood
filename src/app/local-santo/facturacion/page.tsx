    ...
    <main className="min-h-screen bg-[var(--brand-cream)] px-4 py-8 text-[var(--brand-ink-2)]">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase text-[var(--brand-ink-3)]">Facturación</h1>
          ...
          <>
            {billable.length > 0 && (
              <div className="mt-6 grid gap-2 rounded-2xl border-2 border-[var(--brand-primary)]/20 bg-white p-4 sm:grid-cols-2">
                ...
                <label className="flex items-center gap-2 rounded-xl border-2 border-[var(--brand-primary)]/25 bg-white px-4 py-3 font-bold">
                  <span className="text-xs font-black uppercase text-[var(--brand-primary)]">IVA %</span>
                ...
                <button onClick={createInvoice} disabled={busy || !reservationId} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[var(--brand-primary)] px-4 py-3 text-sm font-black uppercase text-white disabled:opacity-50 sm:col-span-2">
            ...
            {loading ? (
              ...
              <ul className="mt-8 space-y-3">
                {invoices.map((inv) => (
                  <li key={inv.id} className="rounded-2xl border-2 border-[var(--brand-primary)]/20 bg-white p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-lg font-black text-[var(--brand-ink-3)]">Factura {inv.serie}-{String(inv.number).padStart(5, "0")}</p>
                      ...
                      <span className="text-2xl font-black text-[var(--brand-ink-3)]">${inv.total}</span>
                    </div>
                  </li>
                ))}
              </ul>
