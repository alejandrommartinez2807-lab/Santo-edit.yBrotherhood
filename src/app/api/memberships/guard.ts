...
export async function checkMembershipsAccess(request: NextRequest, allowedRoles: LocalRole[]) {
  ...
  if (!allowedRoles.includes(access.role) || !canLocalAccessUseModule(access, "guestMemberships")) {
    return forbidden("Esta clave no tiene permiso para membresías")
  }
  const businessConfig = await getBusinessConfig()
  const a = getModulePlanAccess(businessConfig, "guestMemberships")
  if (!a.includedInPlan) return forbidden(`Membresías está disponible desde ${a.minimumPlanLabel}.`)
  if (!a.effectiveEnabled) return forbidden("Membresías está desactivada para este negocio.")
