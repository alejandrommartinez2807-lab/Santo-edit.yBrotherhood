...
export async function checkNotificationsAccess(request: NextRequest, allowedRoles: LocalRole[]) {
  ...
  if (!allowedRoles.includes(access.role) || !canLocalAccessUseModule(access, "guestNotifications")) {
    return forbidden("Esta clave no tiene permiso para notificaciones")
  }
  const businessConfig = await getBusinessConfig()
  const a = getModulePlanAccess(businessConfig, "guestNotifications")
  if (!a.includedInPlan) return forbidden(`Notificaciones está disponible desde ${a.minimumPlanLabel}.`)
  if (!a.effectiveEnabled) return forbidden("Notificaciones está desactivado para este negocio.")
