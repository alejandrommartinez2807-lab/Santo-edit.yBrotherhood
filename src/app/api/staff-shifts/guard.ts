...
export async function checkStaffShiftsAccess(request: NextRequest, allowedRoles: LocalRole[]) {
  ...
  if (!allowedRoles.includes(access.role) || !canLocalAccessUseModule(access, "staffShifts")) {
    return forbidden("Esta clave no tiene permiso para turnos")
  }
  const businessConfig = await getBusinessConfig()
  const a = getModulePlanAccess(businessConfig, "staffShifts")
  if (!a.includedInPlan) return forbidden(`Turnos está disponible desde ${a.minimumPlanLabel}.`)
  if (!a.effectiveEnabled) return forbidden("Turnos está desactivado para este negocio.")
