...
export async function checkOdooAccess(request: NextRequest, allowedRoles: LocalRole[]) {
  ...
  if (!allowedRoles.includes(access.role) || !canLocalAccessUseModule(access, "odooSync")) {
    return forbidden("Esta clave no tiene permiso para la integración con Odoo")
  }
  const businessConfig = await getBusinessConfig()
  const a = getModulePlanAccess(businessConfig, "odooSync")
  if (!a.includedInPlan) return forbidden(`La integración con Odoo está disponible desde ${a.minimumPlanLabel}.`)
  if (!a.effectiveEnabled) return forbidden("La integración con Odoo está desactivada para este negocio.")
