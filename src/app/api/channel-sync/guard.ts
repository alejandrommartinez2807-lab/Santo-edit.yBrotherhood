...
export async function checkChannelSyncAccess(request: NextRequest, allowedRoles: LocalRole[]) {
  ...
  if (!allowedRoles.includes(access.role) || !canLocalAccessUseModule(access, "channelManager")) {
    return forbidden("Esta clave no tiene permiso para canales/OTAs")
  }
  const businessConfig = await getBusinessConfig()
  const a = getModulePlanAccess(businessConfig, "channelManager")
  if (!a.includedInPlan) return forbidden(`Canales/OTAs está disponible desde ${a.minimumPlanLabel}.`)
  if (!a.effectiveEnabled) return forbidden("Canales/OTAs está desactivado para este negocio.")
