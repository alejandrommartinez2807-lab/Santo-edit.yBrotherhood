...
export function CloseDetailSection({
  ...
  return (
    <details
      open={defaultOpen}
      className="group rounded-[1.4rem] border border-[var(--brand-border)] bg-[var(--brand-cream)] p-4"
    >
      <summary className="flex cursor-pointer list-none flex-col gap-2 sm:flex-row sm:items-center sm:justify-between [&::-webkit-details-marker]:hidden">
        ...
        <span className="inline-flex w-fit rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-surface-2)] px-4 py-2 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[var(--brand-primary)] group-open:bg-[var(--brand-accent)] group-open:text-[var(--brand-ink)]">
    ...
    <div className="rounded-[1.4rem] border border-[var(--brand-border)] bg-[var(--brand-surface-2)] p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="w-fit rounded-full border border-[var(--brand-primary)]/40 bg-[var(--brand-cream)] px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-primary)]">
      ...
      {isVisible ? (
        ...
        <div className="mt-4 rounded-2xl border border-yellow-400 bg-[rgba(var(--brand-primary-rgb),0.12)] px-4 py-3">
...
export function CloseReviewItemCard({ item }: { item: CloseReviewItem }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 ${classes.wrapper}`}>
      <div className="flex gap-3">
        <div
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border ${classes.icon}`}
        >
          {item.tone === "success" ? (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 px-4 py-4 backdrop-blur-sm sm:items-center">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-[var(--brand-primary)]/45 bg-[var(--brand-cream)] text-[var(--brand-ink-3)] shadow-2xl shadow-black/45">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-[var(--brand-primary)] bg-[var(--brand-surface-2)] px-6 py-5">
          <h2 className="font-serif text-3xl leading-tight text-[var(--brand-ink-3)] font-semibold">
            {title}
          </h2>
          <button
            type="button"
